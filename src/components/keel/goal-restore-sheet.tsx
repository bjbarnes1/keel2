...
export function GoalRestoreSheet({
  open,
  onClose,
  skipId,
  goalName,
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        ...
        setError(cause instanceof Error ? cause.message : "Could not restore transfer.");
      }
    });
  }
      ...
      <div className="glass-heavy relative z-10 w-full max-w-md overflow-hidden rounded-[var(--radius-lg)] border border-white/12 p-0 shadow-[0_24px_64px_rgba(0,0,0,0.45)]">
        ...
        <div className="p-5 pt-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-[color:var(--keel-ink-3)]">Restore transfer</p>
              {goalName ? (
                <h2 className="mt-1 text-lg font-semibold text-[color:var(--keel-ink)]">{goalName}</h2>
              ) : (
                <h2 className="mt-1 text-lg font-semibold text-[color:var(--keel-ink)]">Goal skip</h2>
              )}
              <p className="mt-2 text-sm leading-relaxed text-[color:var(--keel-ink-3)]">
                Resume this modeled transfer on your timeline.
              </p>
            </div>
            <button
              type="button"
